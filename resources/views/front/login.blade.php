...
">
        ...
        <div class="item-name pt-3">
            ...
            <p style="margin: 0;">
            <h4 class="text-center mb-6">Login Here</h4>
            </p>
    ...
    @if (!session('otp_sent'))
    <!-- OTP Send Form -->
    <p class="srp">Please login or sign up</p>
    <form method="POST" action="{{ route('front.user.login') }}" class="p-4 bg-light rounded shadow-sm">
        @csrf
        <!-- Email Input -->
    ...
    <p class="srp">Please check your email. OTP has been sent to your mail.</p>
    <form method="POST" action="{{ route('front.verify.otp') }}" class="p-4 bg-light rounded shadow-sm">
        @csrf
        <!-- Email (Readonly) Input -->
        <div class="form-group mb-3">
            <input type="text" value="{{ session('email') }}" class="form-control" readonly placeholder="Your Email">
        </div>
